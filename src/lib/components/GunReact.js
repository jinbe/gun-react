...
const useGun = (Comp, config = null) => {
  return class createHoc extends React.Component {
    constructor(props) {
      ...
      this.loadScript6 = this.loadScript6.bind(this);
    ...
    componentDidMount() {
      this.loadscriptAndUpdateState();
      this.state.scriptLoaded && this.state.scriptLoaded.then(() => {});
    }
    ...
    loadScript6(callback) {
      const script = document.createElement("script");
      script.src = "https://cdn.jsdelivr.net/npm/gun/lib/sea.js";
      document.getElementsByTagName("head")[0].appendChild(script);
      if (script.readyState) {
        // IE
        script.onreadystatechange = () => {
        ...
        <div>
          {!requesting && (
            <Comp
              {...this.props}
              config={config}
              gunService={this.state.GunReact}
            />
          )}
        </div>
      );
